...
CREATE TABLE IF NOT EXISTS ingredient_density_types (
...
CREATE TABLE IF NOT EXISTS ingredient_density_keywords (
    id       UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    type_id  UUID NOT NULL REFERENCES ingredient_density_types(id) ON DELETE CASCADE,
    keyword  TEXT NOT NULL UNIQUE
);

CREATE INDEX IF NOT EXISTS idx_ingredient_density_keywords_type_id ON ingredient_density_keywords(type_id);

-- Seed: Typen (use ON CONFLICT to skip if already exists)
